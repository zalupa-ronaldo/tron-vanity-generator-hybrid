...
bool parse(int argc, char** argv, Options& o) {
    ...
    try {
        for (int i = 1; i < argc; ++i) {
            ...
            if (a == "--seconds") o.seconds = std::stoull(next(i, "--seconds"));
            ...
            else if (a == "--vulkan-field") {
                o.vulkanFieldExplicit = true;
                const auto value = next(i, "--vulkan-field");
                if (value != "10x26" && value != "8x32")
                    throw std::runtime_error("--vulkan-field must be 10x26 or 8x32");
                o.vulkanField8 = value == "8x32";
            }
                ...
                o.vulkanResidentDispatches = std::stoul(next(i, "--vulkan-resident-group"));
                if (o.vulkanResidentDispatches != 4 && o.vulkanResidentDispatches != 8 &&
                    o.vulkanResidentDispatches != 16)
                    throw std::runtime_error("--vulkan-resident-group must be 4, 8 or 16");
            }
            ...
            else if (a == "--opencl-curve-batch") {
                o.openclOptions.curveBatch = std::stoul(next(i, "--opencl-curve-batch"));
                if (o.openclOptions.curveBatch != 2 && o.openclOptions.curveBatch != 4 &&
    ...
    if (o.vulkanFieldExplicit && o.backend != "vulkan") {
        std::cerr << "--vulkan-field requires --backend vulkan\n"; return false;
    }
    if (o.vulkanBatchKeysExplicit && o.backend != "vulkan") {
        std::cerr << "--vulkan-batch-keys requires --backend vulkan\n"; return false;
    }
    if (o.vulkanResidentDispatchesExplicit && o.backend != "vulkan") {
        std::cerr << "--vulkan-resident-group requires --backend vulkan\n"; return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (std::find(effective.begin(), effective.end(), "--vulkan-keccak-test") != effective.end()) {
        ...
        return stageResult == 0 ? vulkanResidentSelfTest(opt.vulkanField8) : stageResult;
    }
    ...
    for (size_t i = 1; i < effective.size(); ++i) {
        if (effective[i] == "--gputest") {
            if (opt.backend == "vulkan") return vulkanResidentSelfTest(opt.vulkanField8);
            if (opt.backend == "cuda") {
    ...
    if (opt.vulkanProfile) {
        ...
        const auto p = profileVulkanResident(dictionary, opt.benchSeconds, opt.vulkanCurveBatch,
                                             ...
                                             opt.vulkanResidentDispatches, opt.vulkanField8);
        ...
        std::cout << "Vulkan full-address profile (no wallets): " << p.device << "\n"
                  ...
                  << "field representation: " << p.fieldRepresentation << "\n"
                  ...
                  << "GPU stage buffers: " << (p.gpuScratchDeviceLocal ?
                                                  "GPU-only device-local" :
                                                  "GPU-only non-device-local") << "\n"
        ...
        const std::array<const char*, 5> hostNames = {"setup", "record", "submit", "fence wait", "collect"};
        std::cout << "Host wall intervals (fence wait includes queue + GPU; not additive with GPU timestamps):\n";
        for (size_t i = 0; i < hostNames.size(); ++i)
            std::cout << "  " << hostNames[i] << ": " << p.hostSeconds[i] << " s\n";
        std::cout << "candidate records collected: " << p.candidateRecords << "\n";
        return 0;
    }
